...
class MenuLeft extends React.Component {
  ...
  generateMenuItems = () => {
    ...
    const generateItem = item => {
      const { key, title, url, icon, disabled, pro, heading } = item
      if (item.divider) {
        return <Divider key={Math.random()} />
      }
      if (item.heading) {
        return (
          <Menu.Item key={Math.random()} disabled={disabled}>
            {icon && <span className={`${icon} ${styles.icon} icon-collapsed-hidden`} />}
            <span className={`${styles.title} font-size-16 manu-head`}>{heading}</span>
          </Menu.Item>
        )
      }
      if (item.url) {
          ...
          <Menu.Item key={key} disabled={disabled}>
            {item.target ? (
              <a href={url} target={item.target} rel="noopener noreferrer">
                ...
                {pro && (
                  <span className="badge badge-primary badge-collapsed-hidden ml-2">New</span>
                )}
              </a>
            ) : (
              <Link to={url}>
      ...
      return (
        <Menu.Item key={key} disabled={disabled}>
          {icon && <span className={`${icon} ${styles.icon} icon-collapsed-hidden`} />}
          <span className={styles.title}>{title}</span>
          {pro && <span className="badge badge-primary badge-collapsed-hidden ml-2">New</span>}
        </Menu.Item>
      )
    }
    ...
    return menuData.map(menuItem => {
      ...
      if (menuItem.children) {
        ...
        return (
          <SubMenu title={subMenuTitle} key={menuItem.key} theme={isLightTheme ? 'light' : 'dark'}>
            {generateSubmenu(menuItem.children)}
          </SubMenu>
        )
      }
      ...
      >
        <div className={styles.logo}>
          <div className={styles.logoContainer}>
            <img
              src={`resources/images/logo_square${menuSettings.collapsed ? '-mobile' : ''}.svg`}
              alt=""
            />
          </div>
        </div>
