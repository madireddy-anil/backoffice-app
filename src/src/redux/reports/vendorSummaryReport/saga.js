...
const prepareTableColumns = data => {
  ...
  for (let object = 0; object < data.length; object += 1) {
    ...
    tableColumns.push(vendorColumn)
    tableColumns.push({
      title,
      children: [
        {
...
const fillRemainingColumns = (data, dataIndex, filledIndex) => {
  const emptySpaces = {}
  const nullSymbol = 0
  for (let columnIndex = 0; columnIndex < dataIndex.length; columnIndex += 1) {
    if (filledIndex !== dataIndex[columnIndex]) {
...
/**
 *
 * @function classifyDataByVendor
 *
 * Iterate over the data, extract unique items.
 *
 * @param data : array : initial data fetch from server.
 *
 * @returns : Object : distinct year, month, week, vendor name arrays.
 *
 */

const classifyDataByVendor = data => {
  const vendors = _.uniq(
    data.map(obj => {
  ...
  for (let vendor = 0; vendor < vendors.length; vendor += 1) {
    ...
    const weeks = []
    for (let object = 0; object < data.length; object += 1) {
      if (data[object].vendorName === vendors[vendor]) {
        months.push(data[object].month)
        years.push(data[object].year)
        weeks.push(data[object].week)
      }
    }
    collection.push({
...
const prepareValueCustomizeValue = data => {
  ...
  for (let object = 0; object < data.length; object += 1) {
    if (data[object].week) {
      week.push(data[object].week.toString())
    }
    month.push(data[object].month)
    year.push(data[object].year.toString())
    if (data[object].vendorName) vendorName.push(data[object].vendorName)
  }
  ...
  return {
    ...
    year: _.uniq(year),
    week: _.uniq(week),
    // This is Actually a vendor only, for convience it used as clients....
    clients: _.uniq(vendorName),
  }
}
...
const modifyResponseAndTableColumn = (data, selectedPeriod) => {
  ...
  const getCollectionOfVendor = classifyDataByVendor(data)

  const newResponse = []

  for (let vendor = 0; vendor < getCollectionOfVendor.length; vendor += 1) {
    for (let year = 0; year < getCollectionOfVendor[vendor].years.length; year += 1) {
      for (let month = 0; month < getCollectionOfVendor[vendor].months.length; month += 1) {
        for (let week = 0; week < getCollectionOfVendor[vendor].weeks.length; week += 1) {
          const filledIndices = []
          const newObject = {}
          newObject.year = getCollectionOfVendor[vendor].years[year]
          newObject.month = getCollectionOfVendor[vendor].months[month]
          newObject.week = getCollectionOfVendor[vendor].weeks[week]
          newObject.vendorName = getCollectionOfVendor[vendor].vendorName
          for (let object = 0; object < data.length; object += 1) {
            if (
              data[object].year === getCollectionOfVendor[vendor].years[year] &&
              data[object].month === getCollectionOfVendor[vendor].months[month] &&
              data[object].week === getCollectionOfVendor[vendor].weeks[week] &&
              data[object].vendorName === getCollectionOfVendor[vendor].vendorName
            ) {
          ...
          const obj =
            filledIndices.length > 0
              ? fillRemainingColumns(
                  newObject,
                  _.difference(tableColumnForDataIndex, filledIndices),
                )
              : {}
  ...
  if (selectedPeriod.includes('yearly')) {
    startIndex = 2
  }
...
const getReportData = values => {
  ...
  return reportsPrivatePost(`vendor-summary-report`, body, value.token).then(async res => {
...
export function* handleReport(values) {
  try {
    ...
    yield put({
      type: actions.GET_VENDOR_SUMMARY_REPORT_SUCCESS,
      value: response,
    })
  } catch (exception) {
    yield put({
      type: actions.GET_VENDOR_SUMMARY_REPORT_FAILURE,
      value: exception,
    })
  }
}
...
export default function* rootSaga() {
  yield all([takeEvery(actions.GET_VENDOR_SUMMARY_REPORT, handleReport)])
}
