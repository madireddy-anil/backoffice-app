...
const getallTatPortfolioReport = (value, token) => {
  ...
  return reportsPrivatePost(`tat-summary-report`, body, token).then(response => {
    ...
    return {
      ...
      customizeValue: { year: _.uniq(year), month: _.uniq(month), customizeColumns },
    }
  })
}

export function* getTatPortfolioReportList(values) {
  try {
    const response = yield call(getallTatPortfolioReport, values.value, values.token)
    yield put({
      type: actions.GET_TAT_PORTFOLIO_REPORT_SUCCESS,
      value: response,
    })
  } catch (e) {
    yield put({
      type: actions.GET_TAT_PORTFOLIO_REPORT_FAILURE,
      value: e,
    })
    notification.error({
...
export default function* rootSaga() {
  yield all([takeEvery(actions.GET_TAT_PORTFOLIO_REPORT, getTatPortfolioReportList)])
}
