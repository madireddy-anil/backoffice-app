...
export function* getAllTradesList(values) {
  try {
    ...
    yield put({
      type: actions.GET_TRADES_SUCCESS,
      value: response,
    })
  } catch (e) {
    yield put({
      type: actions.GET_TRADES_FAILURE,
      value: e,
    })
    notification.error({
...
export function* deleteTrade(values) {
  try {
    ...
    yield put({
      type: actions.DELETE_TRADE_SUCCESS,
      value: response,
    })
    yield put({
    ...
    yield put({
      type: actions.DELETE_TRADE_FAILURE,
      value: err,
    })
    notification.error({
      message: globalMessages.errorMessage,
...
export function* bulkDeleteTrades(values) {
  try {
    ...
    yield put({
      type: actions.BULK_DELETE_TRADES_SUCCESS,
      value: response,
    })
    yield put({
      type: actions.GET_TRADES,
    ...
    yield put({
      type: actions.BULK_DELETE_TRADES_FAILURE,
      value: err,
    })
    notification.error({
      message: globalMessages.errorMessage,
...
// const getAllTradesDownload = (values, token) => {
//   // const limit = values.limit ? encodeURI(`limit=${values.limit}`) : ''
//   return txnPrivateGet(`tx-service/trades?limit=${values.limit}`, token).then(response => {
//     return response.data.data
//   })
// }
...
export function* getTradesBulkDownload(values) {
  try {
    ...
    yield put({
      type: actions.GET_TRADES_BULK_DOWNLOAD_SUCCESS,
      value: response,
    })
  } catch (e) {
    yield put({
      type: actions.GET_TRADES_BULK_DOWNLOAD_FAILURE,
      value: e,
    })
    notification.error({
      message: globalMessages.errorMessage,
...
export default function* rootSaga() {
  yield all([
    takeLatest(actions.DELETE_TRADE, deleteTrade),
    // takeLatest(actions.DELETE_TRADE, deleteTradeProgressPage),
    takeEvery(actions.GET_TRADES, getAllTradesList),
    takeLatest(actions.BULK_DELETE_TRADES, bulkDeleteTrades),
    takeLatest(actions.GET_TRADES_BULK_DOWNLOAD, getTradesBulkDownload),
  ])
}
