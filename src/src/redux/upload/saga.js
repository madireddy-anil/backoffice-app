...
export function* initiateUpload(files) {
  ...
  try {
    ...
    yield put({
      type: actions.INITIATE_UPLOAD_SUCCESS,
      value: response,
    })
  } catch (e) {
    yield put({
      type: actions.INITIATE_UPLOAD_FAILURE,
    })
    NProgress.done()
    notification.error({
...
export function* createDepositFileInTrade(values) {
  ...
  try {
    ...
    if (category === 'merchant' || category === 'introducer' || category === 'indvidual') {
      yield put({
        type: actions.CREATE_DEPOSIT_SLIP_SUCCESS,
        value: response,
      })
      const progressData = {
        progressLogs: {
    ...
    yield put({
      type: actions.CREATE_DEPOSIT_SLIP_FAILURE,
      value: err,
    })
    notification.error({
      message: globalMessages.errorMessage,
...
export function* getPreSignedUrl(values) {
  ...
  try {
    ...
    yield put({
      type: actions.GET_PRE_SIGNED_URL_SUCCESS,
      value: response,
    })
    NProgress.done()
  } catch (err) {
...
export default function* rootSaga() {
  yield all([
    takeLatest(actions.INITIATE_UPLOAD, initiateUpload),
    takeLatest(actions.CREATE_DEPOSIT_SLIP, createDepositFileInTrade),
    takeLatest(actions.GET_PRE_SIGNED_URL, getPreSignedUrl),
  ])
}
