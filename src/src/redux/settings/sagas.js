...
export function* SETUP() {
  ...
  const isTabView = (load = false) => {
    const currentState = global.window.innerWidth <= 992 && global.window.innerWidth >= 768
    const prevState = store.get('app.settings.isTabView')
    if (currentState !== prevState || load) {
      reduxStore.dispatch({
        type: 'settings/CHANGE_SETTING',
        payload: {
