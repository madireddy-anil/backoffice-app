...
const getBeneficiaries = values => {
  return txnPrivateGet(
    `tx-service/beneficiaries?clientId=${values.value}&beneStatus=active&limit=0`,
    values.token,
  ).then(response => {
...
export function* getAllBeneficiariesById(values) {
  try {
    ...
    yield put({
      type: actions.GET_BENEFICIARY_BY_CLIENT_ID_SUCCESS,
      value: response.beneficiaries,
    })
  } catch (err) {
    yield put({
      type: actions.GET_BENEFICIARY_BY_CLIENT_ID_FAILURE,
      payload: err,
    })
  }
}
...
export function* createNewTrade(values) {
  try {
    ...
    yield put({
      type: actions.CREATE_TRADE_SUCCESS,
      value: response,
    })
    ...
    yield put({
      type: chatActions.CREATE_CHAT_CHANNEL,
      value: values,
      token: values.value.chatToken,
    })
    yield put(push('/trade'))
    yield put({
      type: tradeActions.GET_TRADE_DETAILS_BY_ID,
      value: response.id,
      token: values.token,
    })
  } catch (err) {
    yield put({
      type: actions.CREATE_TRADE_FAILURE,
      payload: err,
    })
    notification.error({
...
export function* updateTradeStatus(values) {
  try {
    ...
    yield put({
      type: actions.UPDATE_TRADE_STATUS_SUCCESS,
      value: response,
    })
  } catch (err) {
    yield put({
      type: actions.UPDATE_TRADE_STATUS_FAILURE,
      payload: err,
    })
    notification.error({
      message: globalMessages.errorMessage,
...
const getBene = values => {
  ...
  return txnPrivateGet(
    `tx-service/crypto-beneficiaries?clientId=${value}&beneStatus=active&limit=0`,
    token,
  ).then(response => {
    return response.data.data
  })
...
export function* getCryptoBeneficiaries(values) {
  try {
    ...
    yield put({
      type: actions.GET_BENEFICIARY_CRYPTO_BY_CLIENT_ID_SUCCESS,
      value: response,
    })
  } catch (err) {
    yield put({
      type: actions.GET_BENEFICIARY_CRYPTO_BY_CLIENT_ID_FAILURE,
      value: err,
    })
  }
}
...
export default function* rootSaga() {
  yield all([
    takeLatest(actions.GET_BENEFICIARY_BY_CLIENT_ID, getAllBeneficiariesById),
    takeLatest(actions.GET_BENEFICIARY_CRYPTO_BY_CLIENT_ID, getCryptoBeneficiaries),
    takeLatest(actions.CREATE_TRADE, createNewTrade),
    takeLatest(actions.UPDATE_TRADE_STATUS, updateTradeStatus),
  ])
}
