...
import {
  getMarginReport,
  getMarginReportBulkDownload,
  handleMarginReportPagination,
  handleMarginReportFitlers,
} from '../../../../redux/reports/marginReport/actions'

import DownloadComponenet from '../../downloadComponents/businessAndFinOpsReportsDownload'
import styles from './style.module.scss'
import jsondata from './data.json'

const mapStateToProps = ({ user, marginReport, settings, general }) => ({
  timeZone: settings.timeZone.value,
  token: user.token,
  marginReport: marginReport.marginReport,
  loading: marginReport.loading,
  pagination: marginReport.pagination,
  clients: general.clients,
  currencies: general.currencies,
  beneficiaries: general.beneficiaries,
  allMarginReportDownload: marginReport.allMarginReportDownload,
  isDownloadDisabled: marginReport.isDownloadDisabled,
  appliedFilters: marginReport.appliedMarginReportFilters,
  downloadData: marginReport.reportDownloadData,
})
...
class MarginReport extends Component {
  constructor(props) {
    ...
    dispatch(handleMarginReportFitlers({ ...appliedFilters, downloadSelectedType: 'current' }))
  ...
  componentDidMount() {
    ...
    dispatch(getMarginReport(value, token))
  }

  componentWillUnmount() {
    this.mounted = false
    const { dispatch } = this.props
    ClearFilterOnComponentUnMount('', handleMarginReportFitlers, dispatch)
  }
  ...
  handleTableChange = pagination => {
    ...
    dispatch(handleMarginReportPagination(pagination))
  }
  ...
  onReset = () => {
    ...
    form.setFieldsValue({
      ...
      customerSellRate: undefined,
      swapProviderName: undefined,
      swapSettlementCurrency: undefined,
      swapBuyRatewithXERate: undefined,
      swapCostInUSD: undefined,
      otcProviderName: undefined,
      otcSettlementCurrency: undefined,
      otcBuyRatewithXERate: undefined,
      otcCostInUSD: undefined,
      liquidateProviderName: undefined,
      liquidateSettlementCurrency: undefined,
      liquidateBuyRatewithXERate: undefined,
      liquidateCostInUSD: undefined,
      fxProviderName: undefined,
      fxSettlementCurrency: undefined,
      tradeStatus: undefined,
      depositCurrency: undefined,
      pcNetMargin: undefined,
      pcNetMarginInUSD: undefined,
    })
    const pagination = {
      current: 1,
      total: 0,
    ...
    dispatch(handleMarginReportFitlers({ downloadSelectedType: 'current' }))
    dispatch(handleMarginReportPagination(pagination))
    dispatch(getMarginReport(values, token))
  }
  ...
  onSubmitFilter = e => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        form.setFieldsValue({
          downloadSelectedType: 'current',
          DateAndTimeForDownload: restrictDateFor45Days(values.DateOfDeposit)
        ...
        dispatch(handleMarginReportFitlers(value))
        dispatch(handleMarginReportPagination(pagination))
  ...
  getFilterDrawer = () => {
    ...
    const clientOption = clients.map(option => (
      <Option
        ...
        value={option.id}
      >
        <h5>{option.genericInformationtradingName}</h5>
  ...
  onChangeDownload = (type, dateFrom, dateTo) => {
    ...
    dispatch(handleMarginReportFitlers({ ...appliedFilters, downloadSelectedType: type }))
    if (type === 'all') {
      filters.limit = 0
      filters.token = token
      dispatch(
        getMarginReportBulkDownload(
          { ...appliedFilters, page: undefined, dateFrom, dateTo, limit: 0, token },
          token,
        ),
      )
  ...
  prepareInitialDownloadData = downloadData => {
    ...
    downloadData.forEach(report => {
      ...
      reportData.push({
        value: report.swapSettlementCurrency ? report.swapSettlementCurrency : nullSymbol,
        style: { alignment: { horizontal: 'center' } },
      })
      reportData.push({
      ...
      reportData.push({
        value: report.otcSettlementCurrency ? report.otcSettlementCurrency : nullSymbol,
        style: { alignment: { horizontal: 'center' } },
      })
      reportData.push({
      ...
      reportData.push({
        value: report.liquidateSettlementCurrency ? report.liquidateSettlementCurrency : nullSymbol,
        style: { alignment: { horizontal: 'center' } },
      })
      reportData.push({
      ...
      reportData.push({
        value: report.fxSettlementCurrency ? report.fxSettlementCurrency : nullSymbol,
        style: { alignment: { horizontal: 'center' } },
      })
      reportData.push({
      ...
      reportData.push({
        value: report.pcNetMargin ? report.pcNetMargin : 0,
        style: { numFmt: '0,0.00\\%', alignment: { horizontal: 'center' } },
      })
      reportData.push({
        value: report.pcNetMarginInUSD ? report.pcNetMarginInUSD : 0,
        style: { numFmt: '0,0.00', alignment: { horizontal: 'center' } },
      })
      reportData.push({
  ...
  handleTableColumns = () => {
    ...
    const columns = [
      ...
      {
        title: 'Destination Currency- Swap',
        dataIndex: 'swapSettlementCurrency',
        key: 'swapSettlementCurrency',
        align: 'center',
        width: 150,
        render: swapSettlementCurrency => swapSettlementCurrency || nullSymbol,
      },
      {
      ...
      {
        title: 'Destination Currency- OTC',
        dataIndex: 'otcSettlementCurrency',
        key: 'otcSettlementCurrency',
        align: 'center',
        width: 150,
        render: otcSettlementCurrency => otcSettlementCurrency || nullSymbol,
      },
      {
      ...
      {
        title: 'Destination Currency- Liquidate',
        dataIndex: 'liquidateSettlementCurrency',
        key: 'liquidateSettlementCurrency',
        align: 'center',
        width: 150,
        render: liquidateSettlementCurrency => liquidateSettlementCurrency || nullSymbol,
      },
      {
      ...
      {
        title: 'Destination Currency- FX',
        dataIndex: 'fxSettlementCurrency',
        key: 'fxSettlementCurrency',
        align: 'center',
        width: 150,
        render: fxSettlementCurrency => fxSettlementCurrency || nullSymbol,
      },
      {
      ...
      {
        title: 'PC Net Margin %',
        dataIndex: 'pcNetMargin',
        key: 'pcNetMargin',
        align: 'center',
        render: pcNetMargin => (pcNetMargin ? `${parseFloat(pcNetMargin)} %` : '0 %'),
      },
      {
        title: 'PC Net Margin USD',
        dataIndex: 'pcNetMarginInUSD',
        key: 'pcNetMarginInUSD',
        align: 'center',
        render: pcNetMarginInUSD => (pcNetMarginInUSD ? amountFormatter(pcNetMarginInUSD) : 0),
      },
      {
        title: 'Trade Status',
  ...
  render() {
    ...
    return (
      <Card
        title={
          <div>
            <span className="font-size-16">Margin Report</span>
          </div>
        }
        bordered
        headStyle={{
      ...
      >
        <Helmet title="Margin Report" />
        <div className={styles.block}>
          {this.defaultFilters()}
          {this.getFilterDrawer()}
          <DownloadComponenet
            titleForDownload="Margin"
