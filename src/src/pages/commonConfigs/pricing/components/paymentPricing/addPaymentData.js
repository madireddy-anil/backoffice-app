...
import { addPaymentPricingData } from 'redux/pricingProfile/action'
...
const mapStateToProps = ({ user, general, pricing }) => ({
  ...
  addPaymentloading: pricing.paymentloading,
})
  ...
  onSubmitPayemntData = event => {
    ...
    form.validateFields((error, values) => {
      if (!error) {
        ...
        values.paymentsActive = true
        dispatch(addPaymentPricingData(values, token))
      }
    })
  }

  render() {
    const { form, currencies, selectedPricingProfile, fromEditView, addPaymentloading } = this.props
          ...
          <div className="col-md-6 col-lg-3">
            <Form.Item label="Payment Type :" hasFeedback>
              {form.getFieldDecorator('type', {
                initialValue: fromEditView ? selectedPricingProfile.payments.type : undefined,
