...
import { disabledFutureDate, restrictDateFor45Days } from 'utilities/transformer'
...
function DefaultFilters(props) {
  const { form, toggleFilter, token, dispatch, applyFilters, onFilterChange } = props
  ...
  const depositDateHandler = () => {
    form.setFieldsValue({
      DateOfInitiate: null,
    })
  }

  const initiateDateHandler = () => {
    form.setFieldsValue({
      DateOfDeposit: null,
    })
  }
