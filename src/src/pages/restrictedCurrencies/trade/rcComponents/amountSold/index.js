...
const mapStateToProps = ({ user, npTransactions }) => ({
  token: user.token,
  isEditTxnMode: npTransactions.isEditTxnMode,
  amountSold: npTransactions.selectedTransaction.amountSold,
  isAmountSoldConfirmed: npTransactions.isAmountSoldConfirmed,
  transactionId: npTransactions.selectedTransaction.id,
  confirmAmountSoldLoader: npTransactions.confirmAmountSoldLoader,
...
class AmountSold extends Component {
  ...
  handleAmountSoldChange = e => {
    ...
    let { value } = e.target
    value = value.replace(/,/g, '')
    dispatch(enteredAmountSold(value ? parseFloat(value) : null))
  }
  ...
  handleEditAmountSoldConfirm = () => {
    ...
    form.validateFields((err, value) => {
      if (!err) {
        ...
        dispatch(updateTransactionValues(txnValues, transactionId, token))
      }
    })
  }
                  ...
                  <div>
                    ...
                    <Form.Item label="">
                      {form.getFieldDecorator('amountSold', {
                        ...
                        <div className={styles.amountSoldContainer}>
                          <div className={styles.leftBox}>{depositCurrency}</div>
                          <Input
