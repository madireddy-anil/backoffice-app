...
const mapStateToProps = ({ user, npTrade, settings, general }) => ({
  ...
  deleteTradeRateLoading: npTrade.deleteTradeRateLoading,
  createManualRateLoading: npTrade.createManualRateLoading,
  sellRates: npTrade.sellRates,
  tradeId: npTrade.tradeId,
  clientId: npTrade.clientId,
  token: user.token,
  baseCurrency: npTrade.sourceCurrency,
...
class RouteTable extends Component {
  ...
  onRowSave = (values, index) => {
    ...
    if (index > -1) {
      dispatch(updateSellRate(values[index], tradeId, token))
    }
  }
