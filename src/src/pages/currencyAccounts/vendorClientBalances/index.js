...
class AccountBalances extends Component {
  ...
  componentDidMount() {
    ...
    dispatch(getAllVendorClientAccounts(value, token))
    const type = 'vendor_client'
  ...
  handleCASummary = record => {
    ...
    history.push(`/vendor-client-balance/transactions-account-details/${value.id}`)
  }
  ...
  handleTableChange = currentPage => {
    ...
    dispatch(getCAofVendorClientByFilters(value, token))
  ...
  confirmAddAccount = () => {
    ...
    dispatch(addVendorClientCAByCurrency(value, token))
  }
  ...
  onSubmit = event => {
    ...
    form.validateFields((error, values) => {
      if (!error) {
        ...
        dispatch(addVendorClientCAByCurrency(values, token))
      }
    })
  }
