...
import { initiateManualDebit, updateAdjustmentErrorList } from 'redux/caBalanceAdjustments/actions'
...
const mapStateToProps = ({ user, caTransactions, caBalanceAdjustments, auth0 }) => ({
  ...
  errorList: caBalanceAdjustments.errorList,
})

@withRouter
@Form.create()
@connect(mapStateToProps)
class ManualDebit extends React.Component {
  state = {
    reference: '',
    amount: undefined,
    valueDate: '',
  ...
  componentDidMount() {
    ...
    dispatch(updateAdjustmentErrorList([]))
  }

  onSubmitHandle = () => {
    const { amount, reference, valueDate } = this.state
    const { selectedAccountDetails, dispatch, paymentType, token } = this.props
    const value = {
    ...
    dispatch(initiateManualDebit(selectedAccountDetails.id, value, token))
  ...
  handleAuthroizePayment = event => {
    ...
    form.validateFields((error, values) => {
      if (!error) {
        ...
        dispatch(initiateManualDebit(parameters, token))
      }
    })
  }
  ...
  render() {
    const { form, loading, twoFAauthModal, errorList } = this.props
    const {
      amount,
      displayAmount,
      reference,
    ...
    return (
      <div>
        ...
        <Card style={{ marginBottom: '3px' }}>
          <div className="font-size-18 font-weight-bold">Manual Debit</div>
          <Divider />
          {errorList.length > 0 ? (
            <div>
          ...
          <Steps current={step} size="small" direction="vertical" className={styles.stepsBlock}>
            <Step
              title={<span className={styles.stepTitle}>Debit</span>}
              icon={
                <img
