...
import { getName, getCompanyName } from 'utilities/transformer'
// import lodash from 'lodash'
import { withRouter } from 'react-router'
import { connect } from 'react-redux'
import {
  updateCAInitialDataEditMode,
  editClientPaymentAccount,
...
class editCurrencyAccount extends Component {
  ...
  updateToState = () => {
    ...
    // const selectedBrand = brands.find(brand => brand?.id === selectedCurrencyAccount.productBrandId)
  ...
  //   this.setState({ products: selectedBrand?.products })
  //   form.setFieldsValue({
  //     productId: selectedBrand?.products?.id,
  //   })
  // }
    ...
    form.validateFields((error, values) => {
      if (
        ...
        if (!error) {
          ...
          dispatch(editClientPaymentAccount(record.id, value, token))
        }
      } else {
        notification.error({
                  ...
                  <Form.Item label="Account Owner" hasFeedback>
                    {form.getFieldDecorator('ownerEntityId', {
                      initialValue: record.ownerEntityId
                        ? getName(combinedClientsAndIntroducers, record.ownerEntityId)
                        : '',
                ...
                <Form.Item label="Currency" hasFeedback>
                  {form.getFieldDecorator('currency', {
                    ...
                    <Input
                      ...
                      // onChange={value => this.onChangeCurrencySelected(value)}
                    />,
                  )}
                </Form.Item>
              </div>
              ...
              {/* <div className="col-md-6 col-lg-3">
                <Form.Item label="Associated Brands" hasFeedback>
                  {form.getFieldDecorator('brandId', {
                    initialValue: selectedCurrencyAccount.productBrandId,
              ...
              <div className="col-md-6 col-lg-3">
                <Form.Item label="Associated Products" hasFeedback>
                  {form.getFieldDecorator('productId', {
                    initialValue: selectedCurrencyAccount.productId,
