...
import {
  ...
  deleteClientPaymentAccount,
} from 'redux/currencyAccounts/action'
import { getName, getCompanyName, formatToZoneDate } from 'utilities/transformer'
...
class viewCAInitialData extends Component {
  ...
  //   return lodash.startCase(selectedBrand?.brand)
  // }

  // getBrandCode = brandId => {
  //   const { brands } = this.props
  //   const selectedBrand = brands.find(brand => brand?.id === brandId)
  //   return selectedBrand?.brandCode
  // }

  getProductName = prodId => {
    const { brands } = this.props
    // const selectedBrand = brands?.find(brand => brand?.id === brandId)
    const selectedProduct = brands[0]?.products?.find(product => product?.id === prodId)
  ...
  handleDeleteAccount = accountId => {
    ...
    dispatch(deleteClientPaymentAccount(accountId, token))
  }
            ...
            <div className="pb-4 mt-1">
              <span className="font-size-13">
                {selectedCurrencyAccount.ownerEntityId
                  ? getName(combinedClientsAndIntroducers, selectedCurrencyAccount.ownerEntityId)
                  : '--'}
              </span>
            </div>
          </div>
              ...
              <div className="pb-4 mt-1">
                <span className="font-size-13">
                  {selectedCurrencyAccount.productBrandId
                    ? `${this.getBrandName(
                        selectedCurrencyAccount.productBrandId,
                      )} - ${this.getBrandCode(selectedCurrencyAccount.productBrandId)}`
                    : '--'}
                </span>
              </div>
            </div> */}
