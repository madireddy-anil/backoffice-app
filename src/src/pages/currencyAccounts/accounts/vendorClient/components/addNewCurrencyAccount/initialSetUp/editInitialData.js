...
import { getVendorsName, getCompanyName } from 'utilities/transformer'
// import lodash from 'lodash'
import { withRouter } from 'react-router'
import { connect } from 'react-redux'
import {
  updateCAInitialDataEditMode,
  editVendorClientPaymentAccount,
...
class addNewCurrencyAccount extends Component {
      ...
      if (
        ...
        if (!error) {
          const value = {
            ...
            isNetworkFeeAccount: values.isNetworkFeeAccount === 'yes' || false,
          ...
          dispatch(editVendorClientPaymentAccount(record.id, value, token))
        }
      } else {
        notification.error({
  ...
  render() {
    const { form, record, addAccountLoading, vendors, companies, countries, errorList } = this.props
                ...
                <Form.Item label="Account Issuer" hasFeedback>
                  {form.getFieldDecorator('issuerEntityId', {
                    initialValue: record.issuerEntityId
                      ? getVendorsName(vendors, record.issuerEntityId)
                      : '',
                    rules: [{ required: true, message: 'Please select client' }],
                  })(<Input placeholder="Enter account type" style={{ pointerEvents: 'none' }} />)}
                </Form.Item>
