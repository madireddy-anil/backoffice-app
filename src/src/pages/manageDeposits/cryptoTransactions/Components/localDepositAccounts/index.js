...
import { updateLocalAccountsToTrade, updateBankAccounts } from 'redux/transactions/actions'
...
const mapStateToProps = ({ general, user, trade, settings, cryptoTransaction }) => ({
  token: user.token,
  vendors: general.newVendors,
  selectedVendor: cryptoTransaction.selectedVendor,
  localDepositAccounts: cryptoTransaction.selectedTransaction.localDepositAccounts,
  progressLogs: trade.progressLogs,
  selectedAccountNames: cryptoTransaction.selectedAccountNames,
  bankAccounts: general.bankAccounts,
  allBankAccounts: cryptoTransaction.allBankAccounts,
  selectedTransaction: cryptoTransaction.selectedTransaction,
  isTransactionDetailsFetched: cryptoTransaction.isTransactionDetailsFetched,
  updateBankAccLoading: cryptoTransaction.updateBankAccLoading,
...
class LocalDepositAccounts extends Component {
  ...
  //   const { dispatch, vendorBankAccounts } = this.props
  //   const accounts = []
  //   const namesList = []
  //   const selectedAccounts = []
  //   if (event.length > 0) {
  //     event.forEach(item => {
  //       const acc = vendorBankAccounts.find(el => item.key === el.id)
  ...
  //   const { dispatch, localDepositAccounts, selectedAccountNames } = this.props
  //   localDepositAccounts.splice(index, 1)
  //   const deleteAccNames = selectedAccountNames.filter(el => el !== selectedAccount.nameOnAccount)
  //   dispatch(deleteSelectedAccount(localDepositAccounts, deleteAccNames))
  // }
  ...
  render() {
    ...
    const { isAccountsSelected } = this.state
    // vendorBankAccounts.map(option => console.log(option))
    // const accountOption =
    //   vendorBankAccounts.length > 0 &&
    //   vendorBankAccounts.map(option => (
