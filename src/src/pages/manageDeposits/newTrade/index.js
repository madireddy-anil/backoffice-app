...
} from '../../../redux/newTrade/actions'

import styles from './style.module.scss'

const { Option } = Select

const mapStateToProps = ({ user, general, chat, newTrade, trade }) => ({
  ...
  introducerOrMerchant: newTrade.introducerOrMerchant,
  selectedIntroducer: newTrade.selectedIntroducer,
  selectedMerchant: newTrade.selectedMerchant,
  selectedIntroducerClient: newTrade.selectedIntroducerClient,
  selectedMerchantClient: newTrade.selectedMerchantClient,
  sourceAmount: newTrade.sourceAmount,
  sourceCurrency: newTrade.sourceCurrency,
  depositAccountPreference: newTrade.depositAccountPreference,
  depositsInCorporateAccount: newTrade.depositsInCorporateAccount,
  depositsInPersonalAccount: newTrade.depositsInPersonalAccount,
  selectedBeneficiary: newTrade.selectedBeneficiary,
  tradeConfirmed: newTrade.tradeConfirmed,
  canShowIntroducerClient: newTrade.canShowIntroducerClient,
  canShowMerchantClient: newTrade.canShowMerchantClient,
  loading: newTrade.loading,
  tradeId: trade.tradeId,
  isSettlementPreferenceSelected: newTrade.isSettlementPreferenceSelected,
  introducerOrMerchantProfile: newTrade.introducerOrMerchantProfile,
  isCrypto: newTrade.isCrypto,
  isBeneficiaryFetching: newTrade.isBeneficiaryFetching,
})
...
export class NewTrade extends Component {
  ...
  handleIntroducerChange = value => {
    const { introducers, dispatch, token, form } = this.props
    const clientSelected = introducers.find(item => item.id === value)
    if (clientSelected) {
      if (clientSelected.hasPartnerCompanies) {
  ...
  handleMerchantClientChange = value => {
    const { clients, dispatch, form, token } = this.props
    const clientSelected = clients.find(item => item.id === value)
    if (clientSelected) {
      // dispatch(getMerchantClients(clientSelected.id, token))
  ...
  handleConfirmTrade = () => {
    ...
    const getClientName = () => {
      switch (introducerOrMerchant) {
        case 'introducer':
          if (selectedIntroducer.genericInformation.hasPartnerCompanies) {
            return selectedIntroducerClient.genericInformation.tradingName
          }
          return selectedIntroducer.genericInformation.tradingName

        case 'merchant':
          if (selectedMerchant.genericInformation.hasPartnerCompanies) {
            return selectedMerchantClient.genericInformation.tradingName
          }
          return selectedMerchant.genericInformation.tradingName
  ...
  handleSettlementPreference = value => {
    ...
    let client = introducerOrMerchant === 'introducer' ? selectedIntroducer : selectedMerchant

    if (Object.entries(selectedIntroducerClient).length > 0) {
      client = selectedIntroducerClient
    } else if (Object.entries(selectedMerchantClient).length > 0) {
      client = selectedMerchantClient
    }
  ...
  render() {
    ...
    const introducerOption = introducers.map(client => {
      return (
        <Option key={client.id} label={client.genericInformation.registeredCompanyName}>
          <div className="list-card">
                            ...
                            selectedIntroducer.constructor === Object ? (
                              ...
                              >
                                <div>
                                  <h6>
                                    {selectedIntroducer
                                      ? selectedIntroducer.genericInformation.tradingName
                                      : ''}
                                  </h6>
                                  <div>
                                    <small>
                                      {selectedIntroducer
                                        ? selectedIntroducer.genericInformation
                            ...
                            selectedMerchant.constructor === Object ? (
                              ...
                              >
                                <div>
                                  <h6>
                                    {selectedMerchant
                                      ? selectedMerchant.genericInformation.tradingName
                                      : ''}
                                  </h6>
                                  <div>
                                    <small>
                                      {selectedMerchant
                                        ? selectedMerchant.genericInformation.registeredCompanyName
                                        : ''}
                                    </small>
                                  </div>
                                  <div className={styles.flexSpaceBetween}>
                            ...
                            selectedIntroducerClient.constructor === Object ? (
                              ...
                              >
                                <div>
                                  <h6>
                                    {selectedIntroducerClient
                                      ? selectedIntroducerClient.genericInformation.tradingName
                                      : ''}
                                  </h6>
                                  <div>
                                    <small>
                                      {selectedIntroducerClient
                                        ? selectedIntroducerClient.genericInformation
                                            .registeredCompanyName
                                        : ''}
                                    </small>
                                  </div>
                            ...
                            selectedMerchantClient.constructor === Object ? (
                              ...
                              >
                                <div>
                                  <h6>
                                    {selectedMerchantClient
                                      ? selectedMerchantClient.genericInformation.tradingName
                                      : ''}
                                  </h6>
                                  <div>
                                    <small>
                                      {selectedMerchantClient
                                        ? selectedMerchantClient.genericInformation
                                            .registeredCompanyName
                                        : ''}
                                    </small>
                                  </div>
