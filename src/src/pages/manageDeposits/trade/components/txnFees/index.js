...
const mapStateToProps = ({ general, user, transactions, settings, trade }) => ({
  vendors: general.newVendors,
  token: user.token,
  rate: transactions.rate,
  createBuyRateLoading: transactions.createBuyRateLoading,
  fxBaseRatesByVendor: transactions.fxBaseRatesByVendor,
  ...
  fxRateByVendorLoader: transactions.fxRateByVendorLoader,
  timeZone: settings.timeZone.value,
  timeZoneCode: settings.timeZone.code,
  sourceCurrency: transactions.selectedTransaction.depositCurrency,
  sourceAmount: transactions.selectedTransaction.totalDepositAmount,
...
class TxnFees extends Component {
  state = {
    rateSelected: false,
  ...
  getFxBaseRatesByVendor = e => {
    ...
    const {
      dispatch,
      token,
      selectedTransaction: { depositCurrency, settlementCurrency },
      selectedVendor,
      form,
    } = this.props
          ...
          <div className="rate-content">
            ...
            <div>
              <span className="font-size-15">
                ...
                <strong>
                  {baseAmount} {depositCurrency} ={' '}
                  {inverseAmount?.$numberDecimal ? inverseAmount?.$numberDecimal : inverseAmount}{' '}
                  {settlementCurrency}{' '}
                </strong>{' '}
        ...
        <div className="rates-card mt-3 p-10">
          ...
          <div className="rate-content">
              ...
              <span className="font-size-15">
                <strong>
                  {`${depositCurrency} ${amountFormatter(totalDepositAmount)} @ ${
                    sellRate?.$numberDecimal ? sellRate?.$numberDecimal : sellRate
                  } = ${settlementCurrency} ${amountFormatter(settlementAmount)}`}
                </strong>
              </span>
            </div>
            <div>
            ...
            <div className="mt-2">
              <h6>Difference:</h6>
              <span>
                {sellRate?.$numberDecimal ? sellRate?.$numberDecimal : sellRate} is{' '}
                <strong>{spreadDifference}%</strong> Over {baseProviderName} Rate{' '}
                {inverseAmount?.$numberDecimal ? inverseAmount?.$numberDecimal : inverseAmount}
              </span>
            </div>
          </div>
          <div className="rate-action">
            <Button
              ...
              disabled={createBuyRateLoading}
              loading={createBuyRateLoading}
            >
              CONFIRM QUOTE
            </Button>
          </div>
          ...
          <div className="rate-content">
            ...
            <div>
              <span className="font-size-15">
                ...
                @ {formatToZoneDate(rateAppliedAt || new Date(), timeZone)} {timeZoneCode}
              </span>
            </div>
          </div>
        </div>
                  ...
                  <span className="font-size-15">
                    <strong>
                      ...
                      @ ${
                        ...
                        settlementAmount
                          ? amountFormatter(formatNumberDecimal(settlementAmount))
                          : '---'
                      }`}
                    </strong>
                  </span>
                ...
                <div className="mt-2">
                  ...
                  <span>
                    {sellRate?.$numberDecimal
                      ...
                      : '---'}{' '}
                    is <strong>{spreadDifference}%</strong> Over {baseProviderName} Rate{' '}
                    {formatNumberDecimal(
                      inverseAmount?.$numberDecimal ? inverseAmount?.$numberDecimal : inverseAmount,
                    )}
                  </span>
  ...
  render() {
    ...
    if (currentTransaction) {
      ...
      isQuoteConfirmed = currentTransaction.progressLogs.quoteConfirmedToVendorAt !== undefined
    }
          ...
          <div>
            ...
            {isQuoteConfirmed ? (
              <div className={styles.prgressContent}>
                {/* <div className="rates-card mt-3 p-10">
                  <h6>Confirmed Rate</h6>
                  <div className="rate-content">
                    <div>
                      <span className="font-size-15">
                        <strong>
                          {sellRate} @ {formatToZoneDate(rateAppliedAt, timeZone)} {timeZoneCode}
                        </strong>
                      </span>
                    </div>
                  </div>
                        ...
                        <strong>
                          ...
                          {fxRate ||
                            (isInverse
                              ...
                              : amountFormatter(checkNumberDecimal(targetAmount)))}{' '}
                          {targetCurrency}{' '}
                        </strong>{' '}
                        @ {formatToZoneDate(rateAppliedAt, timeZone)} {timeZoneCode}
                      </span>
