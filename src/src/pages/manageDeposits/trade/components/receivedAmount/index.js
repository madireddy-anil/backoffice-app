...
const mapStateToProps = ({ user, general, transactions, settings, routingEngine }) => ({
  token: user.token,
  srcCurrencies: general.currencies,
  isEditTxnMode: transactions.isEditTxnMode,
  txnConfirmReceivedAmountLoading: transactions.txnConfirmReceivedAmountLoading,
  depositCurrency: transactions.selectedTransaction.depositCurrency,
  totalDepositAmount: transactions.selectedTransaction.totalDepositAmount,
  ...
  updateTxnAmountLoader: transactions.updateTxnAmountLoader,
  conformReceivedAmountLoader: transactions.conformReceivedAmountLoader,
...
class ReceivedAmount extends Component {
  ...
  updateDepositConfirm = () => {
    ...
    if (routingEngineData.length > 0) {
      if (
        routingEngineData[0].transactionId === transactionId &&
