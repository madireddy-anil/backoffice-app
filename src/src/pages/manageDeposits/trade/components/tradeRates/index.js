...
const mapStateToProps = ({ general, user, trade, settings }) => ({
  clients: general.clients,
  currentClient: general.currentTradeClient,
  currentIntroducer: general.currentIntroducer,
  token: user.token,
  ...
  isPrecisionApply: trade.isPrecisionApply,
  precision: trade.precision,
  isXeDateTimeApply: trade.isXeDateTimeApply,
  rateAppliedAt: trade.rateAppliedAt,
...
class TradeRates extends Component {
  componentDidMount() {
    ...
    const currentClientOrIntroducer = currentClient || currentIntroducer

    if (isDepositAmountConfirmed && Object.entries(rateDetails).length === 0) {
      this.getRate()
    }

    if (currentClientOrIntroducer?.profile?.tradingPreference) {
      const { isInverse, isNewCalculation } = currentClientOrIntroducer?.profile?.tradingPreference
