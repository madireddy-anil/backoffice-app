...
const mapStateToProps = ({ general, trade, settings }) => ({
  ...
  localBankRequestedDate: trade.progressLogs.bankAccountsRequestedByClientAt,
  localBankReceivedDate: trade.progressLogs.bankAccountsSharedToClientAt,
  paySlipRecivedDate: trade.progressLogs.depositSlipsSharedByClientAt,
  depositConfirmedDate: trade.progressLogs.depositConfirmedByClientAt,
  quoteConfirmedDate: trade.progressLogs.quoteConfirmedByClientAt,
  fundsRemittedDate: trade.progressLogs.fundsRemittedToClientAt,
  fundsReceivedDate: trade.progressLogs.fundsRecceiptConfirmationByClientAt,
  ...
  isFundsRemitted: trade.isFundsRemitted,
})

@connect(mapStateToProps)
class TradeSummary extends Component {
  getTradeClient = () => {
    const { clientId, clients, introducers } = this.props
    let clientObj = clients.find(el => el.id === clientId)

    if (!clientObj) {
      clientObj = introducers.find(el => el.id === clientId)
    }
