...
const mapStateToProps = ({ general, user, trades, chat, settings, trade }) => ({
...
class Trades extends Component {
  ...
  componentWillUnmount() {
    ...
    ClearFilterOnComponentUnMount(history.location.pathname, updateTradeFilters, dispatch)
  }
  ...
  onChangeDownload = (e, dateFrom, dateTo, evt) => {
    const {
      token,
      dispatch,
    ...
    if (e === 'all') {
      ...
      dispatch(
        updateTradeFilters({
          ...appliedFilters,
          downloadSelectedType: e,
          DateAndTimeForDownload: evt,
        }),
      )
      dispatch(getTradesBulkDownload(values, token))
    } else {
      values.page = current
      ...
      },
      {
        ...
        render: value => {
          return value === 'completed' || value === 'cancelled' ? (
            <>
              <Icon
                type={value === 'completed' ? 'check-circle' : 'close-circle'}
                style={{
                  color: value === 'completed' ? '#72bb53' : 'red',
                  paddingRight: '10px',
                  paddingLeft: '10px',
                }}
              />
